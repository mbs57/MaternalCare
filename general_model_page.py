...
def render_general_model():
    ...
    with col_right:
        ...
        st.markdown(
            "<p style='font-size:12px; color:#9a8ab0; margin-bottom:0.8rem;'>"
        ...
        # quick normal-range reference
        st.markdown("<div style='margin-top:1.2rem;'>", unsafe_allow_html=True)
        st.markdown(
            "<p style='font-size:11px; color:#b08aba; font-weight:600; margin-bottom:0.3rem;'>"
            "📐 Normal Ranges (reference)</p>",
            unsafe_allow_html=True,
        )
        ranges_text = (
            "Systolic 90–140 mmHg · Diastolic 60–90 mmHg · "
            "BS 3.9–7.8 mmol/L · Temp 97–99 °F · "
            "BMI 18.5–29.9 · HR 60–90 bpm"
        )
        st.markdown(
            f"<p style='font-size:11px; color:#b08aba; line-height:1.7; margin:0;'>{ranges_text}</p>",
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with col_r1:
        ...
        st.markdown(
            ...
            <div class="risk-main-value" style="color:{color}; margin-top:0.7rem;">{nice_label}</div>
            <div class="risk-subtext" style="margin-top:0.4rem;">
                Based on vital signs, blood metrics, and clinical history provided.
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with col_r2:
        if proba is not None:
            ...
            st.progress(conf)
            # Mini probability bars for all classes
            if classes is not None:
                ...
                for c_lbl, p in zip(classes, proba):
                    ...
                    st.markdown(
                        ...
                        f"<div style='width:70px; font-size:11px; color:#8b7b8e;'>{lbl.replace(' Risk','')}</div>"
                        f"<div style='flex:1; background:#f0e8f5; border-radius:999px; height:8px;'>"
    ...
    # ── DiCE + Advice tab (first) ─────────────────────────────────────────────
    with tab_cf:
        X_train, y_train = load_train_data()
        if X_train is None:
            st.warning(
                "Training data not found (`train_df-d3.pkl`). "
                "DiCE needs the original training set to generate counterfactuals. "
                "Ensure the file is placed alongside the app."
            )
            # Still show advice even without DiCE
            st.markdown("### 🩹 Personalised Clinical Advice")
            from utils import render_advice_section
            render_advice_section(
                input_data=input_data,
                normal_ranges=NORMAL_RANGES_DS3,
                feature_advice=FEATURE_ADVICE_DS3,
                shap_values=shap_values,
                feature_names=FEATURES_DS3,
                risk_label=nice_label,
                dice_deltas=None,
            )
        else:
            render_dice_tab(
                model=model,
                x_row=x[0],
                feature_cols=FEATURES_DS3,
                actionable_features=DICE_ACTIONABLE_DS3,
                X_train=X_train,
                y_train=y_train,
                predicted_label=nice_label,
                input_data=input_data,
                normal_ranges=NORMAL_RANGES_DS3,
                shap_values=shap_values,
                feature_names=FEATURES_DS3,
    ...
    with tab_bar:
        ...
        # Plain-English summary before the plot
        ...
        st.markdown(
            "<p style='font-size:13px; color:#5b2d7a; font-weight:600; margin-bottom:0.5rem;'>"
            "📌 Top 3 drivers for this patient: "
            + " · ".join(summary_parts) + "</p>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<p style='font-size:12px; color:#9a8ab0; margin-bottom:0.7rem;'>"
    ...
    with tab_waterfall:
        st.markdown("<div class='shap-card'>", unsafe_allow_html=True)
        st.markdown(
            "<p style='font-size:12.5px; color:#9a8ab0; margin-bottom:0.5rem;'>"
        ...
        fig2 = plot_shap_waterfall(
            shap_values, base_value, x[0], FEATURES_DS3,
    ...
    top5 = idx_sorted_all[:5]
    top_contribs = [(FEATURES_DS3[i], float(shap_values[i])) for i in top5]

    proba_dict = (
        {str(c_): float(p) for c_, p in zip(classes, proba)}
        if (proba is not None and classes is not None) else None
    )
    flagged = get_flagged_features(input_data, NORMAL_RANGES_DS3)

    # Try to get dice_deltas from session state if previously computed
    # (We pass None here if not available — PDF will still show advice without DiCE targets)
    pdf_buffer = create_pdf_report(
        ...
        feature_advice=FEATURE_ADVICE_DS3,
        dice_deltas=None,           # populated below if DiCE was run
        shap_values=shap_values,
        feature_names=FEATURES_DS3,
    )
    ...
    st.markdown(
        "<p style='font-size:11px; color:#c0a0c8; text-align:center; margin-top:0.5rem;'>"
